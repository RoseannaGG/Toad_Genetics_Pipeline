...
VCFfiletest<-read.vcfR("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/766INDIV_3496SNPS.vcf")

# COLUMN WITH NAMES
VCFfiletest@fix[,3]

VCFfiletest_ID<-VCFfiletest@fix[,3]

# remove + and - from name
VCFfiletest_IDedit<-gsub("\\+", "", VCFfiletest_ID)
VCFfiletest_IDedit<-gsub("\\-", "", VCFfiletest_IDedit)

# add ANBO as prefix
VCFfiletest_IDedit<-paste("ANBO_", VCFfiletest_IDedit, sep="")



## edit loci names in vcf file


# find loci in VCFfiletest_ID that match with SNPkeep - grep everything before : and  :
VCFfiletest_ID 

VCFfiletest_ID2<-VCFfiletest_ID 

VCFfiletest_ID2<-as.data.frame(VCFfiletest_ID2)

str(VCFfiletest_ID2)
head(VCFfiletest_ID2)

#VCFfiletest_ID2<-row.names(VCFfiletest_ID2)

VCFfiletest_ID2$SNP<-VCFfiletest_ID2$VCFfiletest_ID2


# remove + and - from name
VCFfiletest_ID2$SNP<-gsub("\\+", "", VCFfiletest_ID2$SNP)
VCFfiletest_ID2$SNP<-gsub("\\-", "", VCFfiletest_ID2$SNP)

VCFfiletest_ID2$SNP<-paste(VCFfiletest_ID2$SNP, sep="","..")
str(VCFfiletest_ID2)
...
pop(gl.toad) <- pop.data$moeregion
...
SNPkeep2<- sub( 'X', '', sub( '-/',  sub( '\\.', '\\/', sub( '\\.', '-', sub( '\\.', ':', SNPkeep)))))




### get current snp list

hwe.keep.loci.names.df<-as.data.frame(hwe.keep.loci.names)

str(hwe.keep.loci.names.df)

"'data.frame':	1526 obs. of  2 variables:
 $ hwe.keep.loci.names: chr  "5597:18:+" "6487:46:+" "13095:16:+" "13454:49:+" ...
 $ SNP                : chr  "X5597:18:.." "X6487:46:.." "X13095:16:.." "X13454:49:.." ..."

head(hwe.keep.loci.names.df)

#hwe.keep.loci.names.df<-row.names(hwe.keep.loci.names.df)

hwe.keep.loci.names.df$SNP<-hwe.keep.loci.names.df$hwe.keep.loci.names
str(hwe.keep.loci.names.df)

# remove + and - from name
hwe.keep.loci.names.df$SNP<-gsub("\\+", "", hwe.keep.loci.names.df$SNP)
str(hwe.keep.loci.names.df)

hwe.keep.loci.names.df$SNP<-gsub("\\-", "", hwe.keep.loci.names.df$SNP)
str(hwe.keep.loci.names.df)

hwe.keep.loci.names.df$SNP<-paste("X", hwe.keep.loci.names.df$SNP, sep="")
str(hwe.keep.loci.names.df)
...
hwe.keep.loci.names.df$SNP<-paste(hwe.keep.loci.names.df$SNP, sep="",".")
str(hwe.keep.loci.names.df)



#turn to df 'data.frame':	3496 obs. of  2 variables:
SNPkeep.df<-as.data.frame(SNPkeep) # 'data.frame':	1526 obs. of  2 variables:
str(SNPkeep.df)

"'data.frame':	1503 obs. of  1 variable:
 $ SNPkeep: chr  "X5597.18.." "X6487.46.." "X13095.16.." "X13454.49.." ...
"
## now match subset hwe.keep.loci.names.df with SNPkeep.df$SNPkeep

SNPkeep.df.FIS<-subset(hwe.keep.loci.names.df, SNP %in% SNPkeep.df$SNPkeep)
str(SNPkeep.df.FIS)


SNPkeep.df.FIS.list<-SNPkeep.df.FIS$hwe.keep.loci.names
str(SNPkeep.df.FIS.list)
" chr [1:1503] "5597:18:+" "6487:46:+" "13095:16:+" "13454:49:+" "18575:14:-" "20152:26:+" "20154:38:-" "32174:12:+" "34438:29:+" "36127:9:-" "38497:47:-" "39925:72:+" ...
"

gl.toad.nosibs.hwe.FIS <- gl.toad.nosibs[ , SNPkeep.df.FIS.list]
...
gl.toad.nosibs.hwe.FIS.R01<-gl.toad.nosibs.hwe.FIS.bypop$R01
gl.toad.nosibs.hwe.FIS.R01
...
gl.toad.nosibs.hwe.FIS.R02<-gl.toad.nosibs.hwe.FIS.bypop$R02
gl.toad.nosibs.hwe.FIS.R02
"/// GENLIGHT OBJECT /////////
...
gl.toad.nosibs.hwe.FIS.R06<-gl.toad.nosibs.hwe.FIS.bypop$R06
gl.toad.nosibs.hwe.FIS.R06
" /// GENLIGHT OBJECT /////////
...
dartR::gl.report.maf(gl.toad.nosibs.hwe.FIS.R06, maf.limit = 0.5, ind.limit = 0, loc.limit = 0,
              v = 2)

### convert genlight to genind objects 
genind.toad.nosibs.hwe.FIS.R01<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R01, probar = FALSE, verbose = NULL)


""


genind.toad.nosibs.hwe.FIS.R02<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R02, probar = FALSE, verbose = NULL)

""


genind.toad.nosibs.hwe.FIS.R06<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R06, probar = FALSE, verbose = NULL)
...
genind2structureL(genind.toad.nosibs.hwe.FIS.R06, file="F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/structure/HG_genind_structure_766INDIV_1503SNPS_regionID.str", pops=TRUE)


genind2structureL(genind.toad.nosibs.hwe.FIS.R01, file="F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/structure/VI_R01_genind_structure_766INDIV_1503SNPS_regionID.str", pops=TRUE)


genind2structureL(genind.toad.nosibs.hwe.FIS.R02, file="F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/structure/ML_R02_genind_structure_766INDIV_1503SNPS_regionID.str", pops=TRUE)
...
##############################
#loading the data file
HG_filtered.VCF<-read.vcfR("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/R06_267indiv_766_2392.vcf")
...
##############################
#loading the data file
VI_filtered.VCF<-read.vcfR("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/R01_33indiv_766_2392.vcf")
...
##############################
#loading the data file
LM_filtered.VCF<-read.vcfR("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/R02_71indiv_766_2392.vcf")
...
####################################
HG_mafvcf.df$Region<-rep("R06")
VI_mafvcf.df$Region<-rep("R01")
LM_mafvcf.df$Region<-rep("R02")
...
### maf - freq no bin
labelsMOEregiongenetics <- c(R01 = "Vancouver Island", R02 = "Lower mainland", R06 = "Haida Gwaii")

png("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/SFS_moeregion_MAF_vs_locicount_766_1503.png", width = 3.5, height = 6.5, units = 'in', res = 300)
ggplot(data=allthreeregions, aes(x=Frequency,fill=Region)) +
  ...
  facet_wrap(~Region,ncol=1,labeller = labeller(Region = labelsMOEregiongenetics))+
...
ggsave("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/SFS_moeregion_MAF_vs_locicount_766_1503.svg", width = 3.5, height = 6.5, units = 'in')
ggplot(data=allthreeregions, aes(x=Frequency,fill=Region)) +
  geom_histogram()+
  scale_fill_manual(values=c("#ab4e03", "#fca45d","#009E73"),
                    name="Region", breaks = c("VanIsland","LowerMainland", "HaidaGwaii","Northwest"), labels=c("Vancouver Island", "Lower mainland", "Haida Gwaii", "Northwest BC"))+
...
keep.loci2 <- nanhwe.mat.df[ which( nanhwe.mat.df$INhwe_count >=17 ), ]
nrow(keep.loci2) #577
...
/// GENLIGHT OBJECT /////////
 ...
 // 766 genotypes,  1,211 binary SNPs, size: 1.3 Mb
 101985 (22.7 %) missing data
 ...
 // Optional content
   @ind.names:  766 individual labels
   @loc.names:  1211 locus labels
   @chromosome: factor storing chromosomes of the SNPs
   @position: integer storing positions of the SNPs
   @pop: population of each individual (group size range: 33-267)
   @other: a list containing: elements without names 
...
##per region
pop(gl.toad.nosibs.hwe) <- pop.data$moeregion
...
hierf.toad.nosibs.hwe$pop <- pop.data$moeregion
hierf.toad.nosibs.hwe$pop <- factor(pop.data$moeregion)
...
write.table(trial, "F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/Fst/Fst_moeregion_766INDIV_1211SNPS_hwe44pops0.01.txt",sep = "\t",row.names = TRUE,col.names = TRUE )
...
#set pop as region
pop(genid.toad.nosibs.hwe)<-pop.data$moeregion
...
R01      0.0735 0.0827  0.0769 0.1036 85.6 -0.0466 -0.0918  0.0000
R02      0.0878 0.0907  0.0903 0.1133 88.2 -0.0281 -0.0742  0.0194
R06      0.0353 0.0778  0.0394 0.1066 44.0 -0.1147 -0.1817 -0.0405
...
SNPkeep2<- sub( 'X', '', sub( '\\.', '\\/', sub( '\\.', '-', sub( '\\.', ':', SNPkeep))))
...
"/// GENLIGHT OBJECT /////////
 ...
 // 766 genotypes,  1503 binary SNPs, size: 1.1 Mb
 62536 (23.03 %) missing data
...
pop(genid.toad.nosibs.hwe.FIS)<-pop.data$moeregion
...
##per region
pop(gl.toad.nosibs.hwe.FIS) <- pop.data$moeregion
toad.pca.scores$pop <- pop(gl.toad.nosibs.hwe.FIS)
...
### convert genlight to genind objects 
gind.toad.nosibs.hwe.FIS.R01<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R01, probar = FALSE, verbose = NULL)


""


gind.toad.nosibs.hwe.FIS.R02<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R02, probar = FALSE, verbose = NULL)

""


gind.toad.nosibs.hwe.FIS.R06<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R06, probar = FALSE, verbose = NULL)

''


### make new pop files for each dataset
pop.data_R01 <- filter(pop.data, moeregion == "R01")
pop.data_R02 <- filter(pop.data, moeregion == "R02")
pop.data_R06 <- filter(pop.data, moeregion == "R06")
...
pop(gind.toad.nosibs.hwe.FIS.R01)<-pop.data_R01$pop

#### heirfstat conversion from genind ########
hierf.toad.nosibs.hwe.FIS.R01<-genind2hierfstat(gind.toad.nosibs.hwe.FIS.R01,pop=NULL)

#has pop - as breeding pond
hierf.toad.nosibs.hwe.FIS.R01$pop


(R01FST<-hierfstat::wc(hierf.toad.nosibs.hwe.FIS.R01,diploid=TRUE))
...
# make pop breeding pond
pop(gind.toad.nosibs.hwe.FIS.R02)<-pop.data_R02$pop

#### heirfstat conversion from genind ########
hierf.toad.nosibs.hwe.FIS.R02<-genind2hierfstat(gind.toad.nosibs.hwe.FIS.R02,pop=NULL)

#has pop - as breeding pond
hierf.toad.nosibs.hwe.FIS.R02$pop


(R02FST<-hierfstat::wc(hierf.toad.nosibs.hwe.FIS.R02,diploid=TRUE))
...
# make pop breeding pond
pop(gind.toad.nosibs.hwe.FIS.R06)<-pop.data_R06$pop

#### heirfstat conversion from genind ########
hierf.toad.nosibs.hwe.FIS.R06<-genind2hierfstat(gind.toad.nosibs.hwe.FIS.R06,pop=NULL)

#has pop - as breeding pond
hierf.toad.nosibs.hwe.FIS.R06$pop


(R06FST<-hierfstat::wc(hierf.toad.nosibs.hwe.FIS.R06,diploid=TRUE))
...
FSTperpop<-cbind(R01FST1,R02FST1,R06FST1)


write.table(FSTperpop[1,], "F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/Fst/FST_within_MOEregion_766_1503.txt")
...
forb2.pop.stats.hwe <- pop.stats(hierf.toad.nosibs.hwe.FIS)
forb2.pop.stats.hwe
...
#write.table(forb2.pop.stats.hwe, "F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/Diversity/hierfstat_popstats_region_766INDIV_1503SNPS_hwe44pops_FISperpond0.1.txt",sep = "\t",row.names = TRUE,col.names = TRUE)
...
### convert genlight to genind objects 
gl.toad.nosibs.hwe.FIS.R01<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R01, probar = FALSE, verbose = NULL)


""


gl.toad.nosibs.hwe.FIS.R02<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R02, probar = FALSE, verbose = NULL)

""


gl.toad.nosibs.hwe.FIS.R06<-dartR::gl2gi(gl.toad.nosibs.hwe.FIS.R06, probar = FALSE, verbose = NULL)

''


############### summary on each one
gl.toad.nosibs.hwe.FIS.R01_sum<-adegenet::summary(gl.toad.nosibs.hwe.FIS.R01)

gl.toad.nosibs.hwe.FIS.R02_sum<-adegenet::summary(gl.toad.nosibs.hwe.FIS.R02)

gl.toad.nosibs.hwe.FIS.R06_sum<-adegenet::summary(gl.toad.nosibs.hwe.FIS.R06)


## rename
divR01<-gl.toad.nosibs.hwe.FIS.R01_sum
divR02<-gl.toad.nosibs.hwe.FIS.R02_sum
divR06<-gl.toad.nosibs.hwe.FIS.R06_sum
...
lsNames=c("R01Hobs","R02Hobs","R06Hobs")
...
R06Hobs<-as.data.frame(R06Hobs)
R02Hobs<-as.data.frame(R02Hobs)
R01Hobs<-as.data.frame(R01Hobs)

Hobsdf <- data.frame(cbind(R02Hobs,R01Hobs,R06Hobs))
...
divR02$R02_Hexp<-divR02$Hexp
divR01$R01_Hexp<-divR01$Hexp
divR06$R06_Hexp<-divR06$Hexp

divallthree<-cbind(divR02$R02_Hexp,divR01$R01_Hexp,divR06$R06_Hexp)
head(divallthree)

## rename column names
names(divallthree)
colnames(divallthree) <- c("R02_Hexp", "R01_Hexp","R06_Hexp")    # Applying colnames
head(divallthree)
str(divallthree)
...
### Hexp means + stdev ######
R01_mean<-mean(dfdivallthree$R01_Hexp) #
R01_sd<-sd(dfdivallthree$R01_Hexp) #

R02_mean<-mean(dfdivallthree$R02_Hexp) #
R02_sd<-sd(dfdivallthree$R02_Hexp) #

R06_mean<-mean(dfdivallthree$R06_Hexp) #
R06_sd<-sd(dfdivallthree$R06_Hexp) 


### save output
Hetsum<-cbind(R01_mean, R01_sd, R02_mean,R02_sd,R06_mean,R06_sd)
...
## reorder
exphet_SEdf$Region<- factor(exphet_SEdf$Region, levels = c("R01_Hexp", "R02_Hexp", "R06_Hexp"))
...
#set pop as region
pop(genid.toadsub.hwe.nolas)<-pop.data$moeregion
...
basicstatmoeregion<-basic.stats(hierf.toadsub.hwe.nolas,diploid=TRUE)
hierf.toad.ind<-basicstatmoeregion$n.ind.samp


### count number indiv per region

numR01<-sum(pop.data$moeregion=="R01") #54
numR02<-sum(pop.data$moeregion=="R02") #104
numR06<-sum(pop.data$moeregion=="R06") #243
...
dfhierf.toad.ind$R01_pct<-dfhierf.toad.ind$R01/numR01
dfhierf.toad.ind$R02_pct<-dfhierf.toad.ind$R02/numR02
dfhierf.toad.ind$R06_pct<-dfhierf.toad.ind$R06/numR06



dfhierf.toad.ind$R01_pctmissing<-(1-(dfhierf.toad.ind$R01/numR01))*100
dfhierf.toad.ind$R02_pctmissing<-(1-(dfhierf.toad.ind$R02/numR02))*100
dfhierf.toad.ind$R06_pctmissing<-(1-(dfhierf.toad.ind$R06/numR06))*100
...
hist(dfhierf.toad.ind$R06_pctmissing)

hist(dfhierf.toad.ind$R01_pctmissing)

hist(dfhierf.toad.ind$R02_pctmissing)

png("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/R01_R02_R06_percentmissingloci_xlim_ylim_601INDIV_601SNPS.png", width = 10, height = 6.5, units = 'in', res = 600)
par(mfrow=c(1,3))
hist(dfhierf.toad.ind$R06_pctmissing,xlim = c(0,100),ylim = c(0,1600))
hist(dfhierf.toad.ind$R01_pctmissing,xlim = c(0,100),ylim = c(0,1600))
hist(dfhierf.toad.ind$R02_pctmissing,xlim = c(0,100),ylim = c(0,1600))
dev.off()



png("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/R01_R02_R06_percentmissingloci_xlim_ylim_40pct_601INDIV_601SNPS.png", width = 10, height = 6.5, units = 'in', res = 600)
par(mfrow=c(1,3))
hist(dfhierf.toad.ind$R06_pctmissing,xlim = c(0,100),ylim = c(0,1600))
...
pop(gl.toad.subset.nolas) <- pop.data$moeregion

gl.toad.subset.nolasbypop<-seppop(gl.toad.subset.nolas)


gl.toad.subset.nolasR01<-gl.toad.subset.nolasbypop$R01
gl.toad.subset.nolasR02<-gl.toad.subset.nolasbypop$R02
gl.toad.subset.nolasR06<-gl.toad.subset.nolasbypop$R06



### convert genlight to genind objects 
gl.toad.subset.nolasR01<-dartR::gl2gi(gl.toad.subset.nolasR01, probar = FALSE, verbose = NULL)


""


gl.toad.subset.nolasR02<-dartR::gl2gi(gl.toad.subset.nolasR02, probar = FALSE, verbose = NULL)

""


gl.toad.subset.nolasR06<-dartR::gl2gi(gl.toad.subset.nolasR06, probar = FALSE, verbose = NULL)

''


############### summary on each one
gl.toad.subset.nolasR01_sum<-adegenet::summary(gl.toad.subset.nolasR01)

gl.toad.subset.nolasR02_sum<-adegenet::summary(gl.toad.subset.nolasR02)

gl.toad.subset.nolasR06_sum<-adegenet::summary(gl.toad.subset.nolasR06)
...
## rename
divR01<-gl.toad.subset.nolasR01_sum
divR02<-gl.toad.subset.nolasR02_sum
divR06<-gl.toad.subset.nolasR06_sum
...
colnames(dfdivallthree) <- c("R02", "R01","R06","locus")  
head(dfdivallthree)
...
pop(genid.toadsub.nolas)<-pop.data$moeregion
...
## rename 
colnames(Hodf) <- c("R01_Hobs", "R02_Hobs","R06_Hobs","locus")  
head(Hodf)


colnames(Hodf) <- c("R01", "R02","R06","locus")  
head(Hodf)
...
labels <- c(R01 = "Vancouver Island", R02 = "Lower mainland",R06="Haida Gwaii")
...
gl.toad.R01<-gl.toad.bypop$R01
gl.toad.R02<-gl.toad.bypop$R02
gl.toad.R06<-gl.toad.bypop$R06



### convert genlight to genind objects 
gl.toad.R01<-dartR::gl2gi(gl.toad.R01, probar = FALSE, verbose = NULL)


""


gl.toad.R02<-dartR::gl2gi(gl.toad.R02, probar = FALSE, verbose = NULL)

""


gl.toad.R06<-dartR::gl2gi(gl.toad.R06, probar = FALSE, verbose = NULL)

''


############### summary on each one
gl.toad.R01_sum<-adegenet::summary(gl.toad.R01)

gl.toad.R02_sum<-adegenet::summary(gl.toad.R02)

gl.toad.R06_sum<-adegenet::summary(gl.toad.R06)
...
## rename
divR01<-gl.toad.R01_sum
divR02<-gl.toad.R02_sum
divR06<-gl.toad.R06_sum
...
## HG ###
FISdataperpondHG<-FISdataperponddf[ , grepl( "R06" , names( FISdataperponddf ) ) ]
...
## VI ###
FISdataperpondVI<-FISdataperponddf[ , grepl( "R01" , names( FISdataperponddf ) ) ]
...
## LM ###
FISdataperpondLM<-FISdataperponddf[ , grepl( "R02" , names( FISdataperponddf ) ) ]
...
png("F:/GBS_data_03_02_21/Lane_1_2_3_feb2023/gstacks_minmapq20_1370/populations_ANBOref_r60_R60pctoverall_mm001_mh06_wss/766INDIV_3496SNPS/NJTREE/NJTREE_colMOEregion_766INDIV_1503SNPS.png", width = 6.5, height = 6.5, units = 'in', res = 300)
